...
) : IConsumer<TransactionRequested>
{
    public async Task Consume(ConsumeContext<TransactionRequested> context)
    {
        ...
        await hub.Clients.Group($"account-{result.AccountId}")
            .SendAsync("NewTransaction", result, ct);

        if (result.RelatedAccountId.HasValue)
        {
            await hub.Clients.Group($"account-{result.RelatedAccountId.Value}")
                .SendAsync("NewTransaction", result, ct);
        }
    ...
    private async Task<TransactionResponse> HandleDeposit(TransactionRequested msg, CancellationToken ct)
    {
        if (msg.Amount <= 0)
        ...
        var tx = AccountHelper.CreateTransaction(account, TransactionType.Deposit, msg.Amount, msg.Description);
    ...
    private async Task<TransactionResponse> HandleWithdraw(TransactionRequested msg, CancellationToken ct)
    {
        if (msg.Amount <= 0)
            throw new ArgumentException("Amount must be positive.");
        ...
        var tx = AccountHelper.CreateTransaction(account, TransactionType.Withdrawal, msg.Amount, msg.Description);
        account.Balance -= msg.Amount;
        tx.BalanceAfter = account.Balance;
    ...
    private async Task<TransactionResponse> HandleDebit(TransactionRequested msg, CancellationToken ct)
    {
        if (msg.Amount <= 0)
            throw new ArgumentException("Amount must be positive.");
        ...
        var tx = AccountHelper.CreateTransaction(account, TransactionType.Debit, msg.Amount, msg.Description);
        account.Balance -= msg.Amount;
        tx.BalanceAfter = account.Balance;
    ...
    private async Task<TransactionResponse> HandleTransfer(TransactionRequested msg, CancellationToken ct)
    {
        if (msg.Amount <= 0)
            throw new ArgumentException("Amount must be positive.");
        ...
        var source = accounts.FirstOrDefault(a => a.Id == msg.AccountId)
            ?? throw new KeyNotFoundException($"Source account {msg.AccountId} not found.");
        var target = accounts.FirstOrDefault(a => a.Id == msg.RelatedAccountId.Value)
        ...
        if (target.Status != AccountStatus.Open)
            throw new InvalidOperationException($"Target account {msg.RelatedAccountId} is not open.");
        ...
        if (string.Equals(source.Currency, target.Currency, StringComparison.OrdinalIgnoreCase))
        {
            ...
            sourceTx = AccountHelper.CreateTransaction(source, msg.Type, msg.Amount, msg.Description);
            targetTx = AccountHelper.CreateTransaction(target, msg.Type, msg.Amount, msg.Description);
        }
        else
        {
            ...
            targetTx = AccountHelper.CreateTransaction(target, msg.Type, targetCreditAmount, msg.Description);
        }
